...
async def get_user_using_token(
    ...
    if expire and int(expire) < datetime.now(timezone.utc).timestamp():
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Time Access token is out"
        )
    ...
    if not user_id:
        raise HTTPException(
            ...
            detail="User ID not found in Access token",
        )
...
def check_user_is_admin(user: User = Depends(get_user_using_token)) -> User:
    if not user.is_admin:
        raise HTTPException(
            ...
            detail="Not allowed. Only admin has access",
        )
    return user
